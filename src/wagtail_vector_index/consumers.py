...
from wagtail_vector_index.models import GetVectorIndexProtocol
...
class WagtailVectorIndexSSEConsumer(AsyncHttpConsumer):
    ...
    page_instance: GetVectorIndexProtocol | None = None
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.page_model_name is None and self.page_instance is None:
            raise ValueError(
                "You must set either the page_model_name or page_instance attribute"
            )

        if self.page_instance is None and self.page_model_name is not None:
            try:
                page_instance = apps.get_model(self.page_model_name)
            except LookupError as e:
                raise ValueError(f"Model {self.page_model_name} not found") from e
            if not isinstance(page_instance, GetVectorIndexProtocol):
                raise ValueError("Page model must implement GetVectorIndexProtocol.")
            self.page_instance = page_instance

        assert isinstance(
            self.page_instance, GetVectorIndexProtocol
        ), "page_instance should implement GetVectorIndexProtocol"

        # Check if the page model has the required method (ModelVectorIndex mixin)
        if not hasattr(self.page_instance, "get_vector_index") or not callable(
            self.page_instance.get_vector_index
        ):
            raise ValueError("Your page_model must implement GetVectorIndexProtocol.")
    ...
    async def handle(self, body):
        ...
        try:
            try:
                ...
                raise ValueError(
                    "User not found in scope, make sure AuthMiddlewareStack is applied correctly"
                ) from e
